	...
	<div id="main">
		...
		$sql = "select * 
				from havarisana_vozila.autodelovi as a
					join havarisana_vozila.enterijer as b
				    on a.id_dela = b.id_dela";
		$result = mysqli_query($conn, $sql);
		if($result){
		?>
			<h3>U ponudi imamo sledeće delove enterijera:</h3>
		<?php	
			while($row = mysqli_fetch_assoc($result)){
				...
				$src = $row["slika_dela"];
				$boja = $row["boja"];
				$karakteristike = $row["tehnicke_karakteristike"];
			...
			?>
				<div class="delovi">
					...
					<p><?php echo $naziv.$br, $cena."RSD".$br, $stanje.$br, $boja.$br, $karakteristike.$br; ?></p>
					<?php 
						if(isset($_SESSION["email"])){
					?>
		...
		?>
			<h3>Trenutno nemamo delove enterijera u ponudi. Hvala.</h3>
		<?php
		}
		?>
	</div>
