...
<div id="wrapper">
	...
	<div id="main">
		...
		$sql = "select * 
				from havarisana_vozila.autodelovi as a
					join havarisana_vozila.delovi_motora as b
				    on a.id_dela = b.id_dela";
		$result = mysqli_query($conn, $sql);
		if($result){
		?>
			<h3>U ponudi imamo sledeće delove motora:</h3>
		<?php	
			while($row = mysqli_fetch_assoc($result)){
				...
				$stanje = $row["stanje"];
				$src = $row["slika_dela"];
				$proizvodjac = ucfirst($row["proizvodjac"]);
				$karakteristike = ucfirst($row["tehnicke_karakteristike"]);
			...
			?>
				<div class="delovi">
					...
					<p><?php echo $naziv.$br, $cena."RSD".$br, $stanje.$br, $proizvodjac.$br, $karakteristike.$br; ?></p>
					<?php 
						if(isset($_SESSION["email"])){
					?>
		...
		?>
			<h3>Trenutno nemamo delove motora u ponudi. Hvala.</h3>
		<?php
		}
		?>
	</div>
	...
	<div id="sidebar">
		...
		<nav id="parts">
			<ul>
				<li><a href="karoserija.php"> Karoserija</a></li>
				<li><a href=""> Delovi motora</a></li>
